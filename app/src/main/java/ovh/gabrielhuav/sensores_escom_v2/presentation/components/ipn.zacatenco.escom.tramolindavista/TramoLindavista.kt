...
/**
 * Activity para el mapa del Tramo Lindavista
 */
class TramoLindavista : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.tramo_lindavista // Usamos la imagen del Tramo Lindavista
            )
...
// Esperar a que el mapView esté listo
            mapView.post {
// Configurar el mapa para el Tramo Lindavista
                mapView.setCurrentMap(MapMatrixProvider.MAP_LINDAVISTA, R.drawable.tramo_lindavista)

// Configurar el playerManager
                mapView.playerManager.apply {
                    setCurrentMap(MapMatrixProvider.MAP_LINDAVISTA)
                    localPlayerId = playerName
                    updateLocalPlayerPosition(gameState.playerPosition)
                }

                Log.d(TAG, "Set map to: " + MapMatrixProvider.MAP_LINDAVISTA)

// Importante: Enviar un update inmediato para que otros jugadores sepan dónde estamos
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, gameState.playerPosition, MapMatrixProvider.MAP_LINDAVISTA)
                }
            }
        } catch (e: Exception) {
...
// Solicitar actualizaciones de posición
                    ...
                    updateBluetoothStatus("Conectado al servidor online - Tramo Lindavista")
                } else {
...
// Interacción con puesto de comida
            if (currentPosition.first == 20 && currentPosition.second == 10) {
                showFoodStand()
            } else {
// Alguna otra interacción disponible en esta posición
    ...
    private fun showFoodStand() {
// Mostrar interacción con puesto de comida
        Toast.makeText(this, "¡Este puesto vende los mejores tacos de canasta de la zona!", Toast.LENGTH_LONG).show()
    }

    private fun returnToPreviousMap() {
// Volver al Tramo Atrás Plaza
...
// Transición al siguiente mapa: TramoTurismo
...
// Enviar la posición con el nombre del mapa correcto
                serverConnectionManager.sendUpdateMessage(playerName, position, MapMatrixProvider.MAP_LINDAVISTA)
            }
...
// Mostrar información sobre la transición disponible
            when (transitionMap) {
                MapMatrixProvider.MAP_TRAS_PLAZA -> {
                    Toast.makeText(this, "Presiona A para volver al Tramo Atrás Plaza", Toast.LENGTH_SHORT).show()
                }
                MapMatrixProvider.MAP_TURISMO -> {
                    Toast.makeText(this, "Presiona A para ir al Tramo Turismo", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
...
// Reconectar si es necesario
    ...
    override fun onPositionReceived(device: BluetoothDevice, x: Int, y: Int) {
        runOnUiThread {
            ...
            mapView.updateRemotePlayerPosition(deviceName, Pair(x, y), MapMatrixProvider.MAP_LINDAVISTA)
            mapView.invalidate()
        }
    }
...
// Obtener el mapa actual normalizado para comparar
                                val currentMap = MapMatrixProvider.normalizeMapName(MapMatrixProvider.MAP_LINDAVISTA)
...
// Reenviar nuestra posición para asegurar que todos nos vean
    ...
    companion object {
        private const val TAG = "TramoLindavista"
    }
}