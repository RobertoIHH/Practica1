...
/**
 * Activity para el mapa del Tramo Turismo
 */
class TramoTurismo : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.tramo_turismo // Usamos la imagen del Tramo Turismo
            )
            ...
            mapView.post {
                // Configurar el mapa para el Tramo Turismo
                mapView.setCurrentMap(MapMatrixProvider.MAP_TURISMO, R.drawable.tramo_turismo)

                // Configurar el playerManager
                mapView.playerManager.apply {
                    setCurrentMap(MapMatrixProvider.MAP_TURISMO)
                    localPlayerId = playerName
                    updateLocalPlayerPosition(gameState.playerPosition)
                }

                Log.d(TAG, "Set map to: " + MapMatrixProvider.MAP_TURISMO)

                // Importante: Enviar un update inmediato para que otros jugadores sepan dónde estamos
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, gameState.playerPosition, MapMatrixProvider.MAP_TURISMO)
                }
            }
        } catch (e: Exception) {
        ...
        serverConnectionManager.connectToServer { success ->
            runOnUiThread {
                ...
                if (success) {
                    ...
                    updateBluetoothStatus("Conectado al servidor online - Tramo Turismo")
                } else {
    ...
    private fun checkForInteraction() {
        ...
        if (transitionPoint != null) {
            ...
            // Manejar interacciones con monumentos
            when {
                currentPosition.first == 10 && currentPosition.second == 20 -> {
                    showMonumentoInfo()
                }
                currentPosition.first == 25 && currentPosition.second == 15 -> {
                    showEstatuaInfo()
                }
                currentPosition.first == 25 && currentPosition.second == 25 -> {
                    showFuenteInfo()
                }
                else -> {
    ...
    private fun showMonumentoInfo() {
        AlertDialog.Builder(this)
            .setTitle("Monumento Histórico")
            .setMessage("Este monumento conmemora la fundación de la Escuela Superior de Cómputo en 1993, una de las mejores escuelas de ingeniería en computación de México.")
            .setPositiveButton("Interesante") { dialog, _ -> dialog.dismiss() }
            .show()
    }

    private fun showEstatuaInfo() {
        AlertDialog.Builder(this)
            .setTitle("Estatua")
            .setMessage("Esta estatua representa el espíritu de innovación y tecnología que caracteriza a la ESCOM.")
            .setPositiveButton("¡Genial!") { dialog, _ -> dialog.dismiss() }
            .show()
    }

    private fun showFuenteInfo() {
        AlertDialog.Builder(this)
            .setTitle("Fuente Pequeña")
            .setMessage("Esta fuente es un lugar popular para estudiantes que buscan un momento de tranquilidad entre clases.")
            .setPositiveButton("Entendido") { dialog, _ -> dialog.dismiss() }
            .show()
    }

    private fun returnToPreviousMap() {
        // Volver al Tramo Lindavista
        val intent = Intent(this, TramoLindavista::class.java).apply {
    ...
    private fun startNextMap() {
        // Transición al siguiente mapa: TramoFuente
        val intent = Intent(this, TramoFuente::class.java).apply {
            putExtra("PLAYER_NAME", playerName)
            putExtra("IS_SERVER", gameState.isServer)
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            ...
            if (gameState.isConnected) {
                ...
                serverConnectionManager.sendUpdateMessage(playerName, position, MapMatrixProvider.MAP_TURISMO)
            }
        ...
        if (transitionMap != null) {
            ...
            when (transitionMap) {
                MapMatrixProvider.MAP_LINDAVISTA -> {
                    Toast.makeText(this, "Presiona A para volver al Tramo Lindavista", Toast.LENGTH_SHORT).show()
                }
                MapMatrixProvider.MAP_FUENTE -> {
                    Toast.makeText(this, "Presiona A para ir al Tramo Fuente", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    override fun onPositionReceived(device: BluetoothDevice, x: Int, y: Int) {
        runOnUiThread {
            ...
            mapView.updateRemotePlayerPosition(deviceName, Pair(x, y), MapMatrixProvider.MAP_TURISMO)
            mapView.invalidate()
        }
    }
                    ...
                    "positions" -> {
                        ...
                        players.keys().forEach { playerId ->
                            if (playerId != playerName) {
                                ...
                                val currentMap = MapMatrixProvider.normalizeMapName(MapMatrixProvider.MAP_TURISMO)
    ...
    companion object {
        private const val TAG = "TramoTurismo"
    }
}